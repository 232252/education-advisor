...
use iced::widget::{container, row, rule, text, Container};
...
pub fn section_header<'a, M: 'a>(title: &str, icon_name: Option<IconName>) -> Element<'a, M> {
    ...
    if let Some(ic) = icon_name {
        children.push(iced::widget::Svg::new(icon(ic)).width(Length::Fixed(14.0)).height(Length::Fixed(14.0)).into());
        children.push(iced::widget::Space::new().width(6).height(0).into());
    }
    children.push(text(title.to_string()).size(14).color(iced::Color::from_rgb(0.95, 0.95, 1.0)).into());
            ...
            .style(|_| iced::widget::container::Style {
                background: Some(iced::Background::Gradient(iced::Gradient::Linear(
                    iced::gradient::Linear::new(iced::Degrees(90.0))
                        .add_stop(0.0, iced::Color::from_rgba(1.0, 1.0, 1.0, 0.10))
                        .add_stop(1.0, iced::Color::TRANSPARENT),
                ))),
                border: iced::Border { color: iced::Color::TRANSPARENT, width: 0.0, radius: iced::border::Radius::from(0.0) },
