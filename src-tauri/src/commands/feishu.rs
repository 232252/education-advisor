...
#[tauri::command]
pub async fn feishu_bitable(state: State<'_, AppState>, app_id: String, app_token: String) -> Result<Value> {
    let secret = state.keystore.get(&format!("feishu:{app_id}"))?.ok_or_else(|| AppError::Feishu("未设置 appSecret".into()))?;
...
#[tauri::command]
pub async fn feishu_send(state: State<'_, AppState>, app_id: String, user_open_id: String, text: String) -> Result<Value> {
    let secret = state.keystore.get(&format!("feishu:{app_id}"))?.ok_or_else(|| AppError::Feishu("未设置 appSecret".into()))?;
    let token = state.feishu.test(&app_id, &secret).await?.0;
    let msg_id = state.feishu.send_text(&token, "open_id", &user_open_id, &text).await?;
    Ok(json!({ "success": true, "messageId": msg_id }))
}

#[tauri::command]
pub async fn feishu_send_preflight(state: State<'_, AppState>, app_id: String, user_open_id: String, text: String) -> Result<PreflightReport> {
...
pub async fn feishu_send_confirm(
    ...
    let msg_id = state.feishu.send_text(&token, "open_id", &user_open_id, &to_send).await?;
...
pub async fn feishu_status(state: State<'_, AppState>) -> Result<String> {
    ...
    Ok(if has_secret { "configured" } else { "not_configured" }.into())
}
...
pub async fn feishu_sync_now(
    ...
    let secret = state.keystore.get(&format!("feishu:{app_id}"))?.ok_or_else(|| AppError::Feishu("未设置 appSecret".into()))?;
    let token = state.feishu.test(&app_id, &secret).await?.0;
    let record_id = state.feishu.bitable_create_record(&token, &app_token, &table_id, fields).await?;
