...
pub async fn privacy_init(
    ...
    let (out, res) = audited(&state, "init", None, None, || {
        ...
        eng.init(&data_dir, &password).map_err(|e| AppError::Privacy(e.to_string()))?;
        if auto_scan.unwrap_or(false) {
            let n = eng.auto_scan_students(&data_dir).map_err(|e| AppError::Privacy(e.to_string()))?;
...
#[tauri::command]
pub async fn privacy_load(state: State<'_, AppState>, password: String) -> Result<EAAResult<Value>> {
    let data_dir = state.paths.eaa_data.clone();
    let (out, res) = audited(&state, "load", None, None, || {
        let mut eng = state.privacy.write();
        eng.load(&data_dir, &password).map_err(|e| AppError::Privacy(e.to_string()))?;
        Ok(json!({ "mappings": eng.mapping_count() }))
    })
    .await;
    match res {
...
#[tauri::command]
pub async fn privacy_enable(app: AppHandle, state: State<'_, AppState>) -> Result<EAAResult<Value>> {
    *state.privacy_enabled.write() = true;
    let _ = broadcaster::emit_all(&app, events::PRIVACY_STATE_CHANGED, json!({ "enabled": true, "at": chrono::Utc::now().timestamp_millis() }));
    Ok(EAAResult::ok(json!({ "enabled": true })))
}

#[tauri::command]
pub async fn privacy_disable(app: AppHandle, state: State<'_, AppState>, _password: String) -> Result<EAAResult<Value>> {
    *state.privacy_enabled.write() = false;
    let _ = broadcaster::emit_all(&app, events::PRIVACY_STATE_CHANGED, json!({ "enabled": false, "at": chrono::Utc::now().timestamp_millis() }));
    Ok(EAAResult::ok(json!({ "enabled": false })))
}

#[tauri::command]
pub async fn privacy_list(state: State<'_, AppState>, _password: String) -> Result<EAAResult<Value>> {
...
#[tauri::command]
pub async fn privacy_add(state: State<'_, AppState>, entity_type: String, text: String) -> Result<EAAResult<Value>> {
    let (out, res) = audited(&state, "add", None, Some(entity_type.clone()), || {
        let et = eaa_core::privacy::EntityType::from_str(&entity_type);
        let mut eng = state.privacy.write();
        let alias = eng.add_entity(&et, &text).map_err(|e| AppError::Privacy(e.to_string()))?;
...
#[tauri::command]
pub async fn privacy_anonymize(state: State<'_, AppState>, text: String) -> Result<EAAResult<Value>> {
...
#[tauri::command]
pub async fn privacy_deanonymize(state: State<'_, AppState>, text: String) -> Result<EAAResult<Value>> {
...
#[tauri::command]
pub async fn privacy_filter(state: State<'_, AppState>, receiver: String, text: String) -> Result<EAAResult<Value>> {
...
#[tauri::command]
pub async fn privacy_backup(state: State<'_, AppState>, dest_path: String) -> Result<EAAResult<Value>> {
    let eng = state.privacy.read();
    eng.backup(&std::path::PathBuf::from(&dest_path)).map_err(|e| AppError::Privacy(e.to_string()))?;
