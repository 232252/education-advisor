...
fn test_stream_event_tags_match_frontend() {
    ...
    // ThinkingDelta
    let v = serde_json::to_value(StreamEvent::ThinkingDelta { delta: "思考".into() }).unwrap();
    assert_eq!(v["type"], json!("thinking_delta"));
    assert_eq!(v["delta"], json!("思考"));

    // Start
    let v = serde_json::to_value(StreamEvent::Start { model: "gpt-4o".into(), provider: "openai".into() }).unwrap();
    assert_eq!(v["type"], json!("start"));
    assert_eq!(v["model"], json!("gpt-4o"));
    assert_eq!(v["provider"], json!("openai"));

    // TextStart / TextEnd
    assert_eq!(serde_json::to_value(StreamEvent::TextStart).unwrap()["type"], json!("text_start"));
    assert_eq!(serde_json::to_value(StreamEvent::TextEnd).unwrap()["type"], json!("text_end"));

    // ThinkingStart / ThinkingEnd
    assert_eq!(serde_json::to_value(StreamEvent::ThinkingStart).unwrap()["type"], json!("thinking_start"));
    assert_eq!(serde_json::to_value(StreamEvent::ThinkingEnd).unwrap()["type"], json!("thinking_end"));

    // ToolcallStart { id, name }
    let v = serde_json::to_value(StreamEvent::ToolcallStart { id: "tc_1".into(), name: "eaa_score".into() }).unwrap();
    ...
    let v = serde_json::to_value(StreamEvent::ToolcallDelta { id: "tc_1".into(), args_delta: "{\"name\":".into() }).unwrap();
    ...
    let v = serde_json::to_value(StreamEvent::ToolResult { id: "tc_1".into(), result: "ok".into(), is_error: false }).unwrap();
    assert_eq!(v["type"], json!("tool_result"));
    assert_eq!(v["isError"], json!(false));

    // Done { usage, cost }
    let v = serde_json::to_value(StreamEvent::Done {
        usage: TokenUsage { input_tokens: 10, output_tokens: 20, cache_read_tokens: 0, cache_write_tokens: 0 },
        cost: 0.5,
    }).unwrap();
    ...
    let v = serde_json::to_value(StreamEvent::Error { message: "超时".into(), retryable: true }).unwrap();
