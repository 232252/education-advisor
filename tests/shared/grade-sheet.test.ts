...
describe('parseGradeSheetMatrix + matchGradeRowsToRoster', () => {
  ...
  it('考号≠学号时按姓名匹配，不新建学生', () => {
    ...
    const { matched, unmatched } = matchGradeRowsToRoster(parsed.rows, [
      { name: '测试甲', studentNumber: '20240005' },
    ])
    expect(matched).toHaveLength(1)
    expect(matched[0]?.matchedName).toBe('测试甲')
  ...
  it('无姓名时可用学号或考号对上', () => {
    ...
    const { matched, unmatched } = matchGradeRowsToRoster(parsed.rows, [
      { name: '测试乙', studentNumber: 'STU-7', examNumber: 'EXAM-7' },
    ])
    expect(unmatched).toHaveLength(0)
    expect(matched[0]?.matchedName).toBe('测试乙')
