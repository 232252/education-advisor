    ...
    const mockApi = {
      eaa: {
        ...
        ranking: vi.fn(async (n: number) => {
          try { const r = await eaaRun(['ranking', String(n), '-O', 'json']); return { success: true, data: JSON.parse(r) } }
          catch (e) { return { success: false, error: String(e) } }
        }),
        summary: vi.fn(async () => {
          try { const r = await eaaRun(['summary', '-O', 'json']); return { success: true, data: JSON.parse(r) } }
          catch (e) { return { success: false, error: String(e) } }
        }),
        stats: vi.fn(async () => {
