...
| **定时任务** | OpenClaw Cron | 18个自动化任务，覆盖全工作日 |
...
| 特性 | 说明 |
|:-----|:-----|
| **PostgreSQL 后端** | MVCC 多版本并发控制，无锁读写 |
| **RLS 多租户隔离** | 数据库行级安全策略，自动隔离不同班级数据 |
| **Append-Only 强制** | 触发器阻止 UPDATE/DELETE，事件不可篡改 |
| **StorageBackend trait** | 文件系统/PostgreSQL 双后端，`EAA_BACKEND` 一键切换 |
| **物化投影表** | 实时分数/排名缓存，查询零延迟 |
| **Docker Compose** | 一键启动 PostgreSQL + EAA |
...
# 切换：PostgreSQL（多班级、高并发）
export EAA_BACKEND=postgres
export DATABASE_URL=postgres://eaa:pass@localhost:5432/eaa
export EAA_TENANT_ID=<uuid>
eaa info
```
