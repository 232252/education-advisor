...
fn print_event_line(evt: &Event, id_to_name: &HashMap<String, String>) {
    let name = id_to_name
        ...
        .map(|s| s.as_str())
        .unwrap_or("?");
    let date = if evt.timestamp.len() >= 10 {
        &evt.timestamp[..10]
    } else {
        &evt.timestamp
    };
...
pub fn cmd_validate(output: OutputMode) -> Result<(), AppError> {
    ...
    let entity_ids: std::collections::HashSet<&str> =
        entities.entities.keys().map(|k| k.as_str()).collect();
...
pub fn cmd_replay(output: OutputMode) -> Result<(), AppError> {
    ...
    match output {
        OutputMode::Json => {
            let ranking: Vec<serde_json::Value> = sorted
                .iter()
                .enumerate()
                .map(|(i, (eid, score))| {
                    let name = ctx
...
pub fn cmd_score(name: &str, output: OutputMode) -> Result<(), AppError> {
    ...
    let risk = entity
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("未知");
    let student_events: Vec<&Event> = ctx
        .events
        .iter()
...
pub fn cmd_add(
    ...
    let new_event = Event {
        ...
        event_type: if delta >= 0.0 {
            EventType::ConductBonus
        } else {
            EventType::ConductDeduct
        },
    ...
    if dry_run {
        println!(
            "[DRY-RUN] event_id:{} student:{} code:{} delta:{:+.1} op:{}",
            new_event.event_id, name, reason_code, delta, op
        );
        return Ok(());
    }
    println!(
        "✓ 事件已创建: {} {} {:+.1}",
        new_event.event_id, name, delta
    );
...
pub fn cmd_revert(
    ...
    can_revert(
        &all_events[target_idx].reverted_by,
        event_id,
        &all_events[target_idx].reason_code,
    )?;
    let entity_id = all_events[target_idx].entity_id.clone();
    let score_delta = all_events[target_idx].score_delta;
    if dry_run {
        println!(
            "[DRY-RUN] target:{} delta:{:+.1}→{:+.1} reason:{}",
            event_id, score_delta, -score_delta, reason
        );
        return Ok(());
    }
    let now = chrono::Local::now()
    ...
    let _ = append_operation_log(
        &serde_json::json!({"action":"revert","revert_id":revert_id,"target_id":event_id,"operator":op,"timestamp":now}),
    );
    Ok(())
}
...
pub fn cmd_codes(output: OutputMode) -> Result<(), AppError> {
    ...
    sorted.sort_by(|a, b| {
        b.1.score_delta
            .unwrap_or(0.0)
            .partial_cmp(&a.1.score_delta.unwrap_or(0.0))
            .unwrap()
    });
    match output {
        OutputMode::Json => {
...
pub fn cmd_search(query: &str, limit: usize, output: OutputMode) -> Result<(), AppError> {
    ...
    for evt in &ctx.events {
        ...
        if name.contains(query)
            || evt.reason_code.contains(&query_upper)
            || evt.category_tags.iter().any(|t| t.contains(query))
            || evt.original_reason.contains(query)
        {
            results.push(evt);
        }
    }
    ...
    match output {
        OutputMode::Json => {
            let items: Vec<serde_json::Value> = results
                .iter()
                .take(limit)
                .map(|e| event_to_json(e, &ctx.id_to_name))
                .collect();
            print_json(
                &serde_json::json!({"query":query,"total":results.len(),"showing":items.len(),"events":items}),
            );
        }
        OutputMode::Text => {
            ...
            println!("{}", "-".repeat(75));
            for evt in results.iter().take(limit) {
                print_event_line(evt, &ctx.id_to_name);
            }
            if results.len() > limit {
                println!("... (共{}条，显示前{}条)", results.len(), limit);
            }
        }
    }
    Ok(())
...
pub fn cmd_stats(output: OutputMode) -> Result<(), AppError> {
    ...
    let valid: Vec<_> = ctx
        .events
        .iter()
        .filter(|e| e.is_valid && e.reverted_by.is_none())
        .collect();
    let reverted_count = ctx
        .events
        .iter()
        .filter(|e| e.reverted_by.is_some())
        .count();
    ...
    intervals.insert("极高(<60)", 0usize);
    intervals.insert("高(60-80)", 0);
    intervals.insert("中(80-100)", 0);
    intervals.insert("低(>=100)", 0);
    for score in ctx.scores.values() {
        let key = if *score < 60.0 {
            "极高(<60)"
        } else if *score < 80.0 {
            "高(60-80)"
        } else if *score < 100.0 {
    ...
    match output {
        OutputMode::Json => {
            let mut code_dist: Vec<serde_json::Value> = code_counts
                .iter()
                .map(|(k, v)| serde_json::json!({"code":k,"count":v}))
                .collect();
            code_dist.sort_by(|a, b| b["count"].as_u64().cmp(&a["count"].as_u64()));
            let mut tag_dist: Vec<serde_json::Value> = tag_counts
                .iter()
                .map(|(k, v)| serde_json::json!({"tag":k,"count":v}))
                .collect();
        ...
        OutputMode::Text => {
            ...
            println!("╠══════════════════════════════════════╣");
            println!("║ 原因码 TOP8:");
            let mut sc: Vec<_> = code_counts.iter().collect();
            sc.sort_by(|a, b| b.1.cmp(a.1));
            for (code, count) in sc.iter().take(8) {
                println!("║   {:<28}{:>3}次", code, count);
            }
            println!("╚══════════════════════════════════════╝");
        }
    }
    ...
    if tag.is_empty() {
        match output {
            ...
            OutputMode::Text => {
                println!("所有标签:");
                println!("{}", "-".repeat(30));
                let mut s: Vec<_> = tag_counts.iter().collect();
    ...
    match output {
        OutputMode::Json => {
            let items: Vec<serde_json::Value> = matched
                ...
                .collect();
            print_json(
                &serde_json::json!({"start":start,"end":end,"total":matched.len(),"showing":items.len(),"events":items}),
            );
        }
        OutputMode::Text => {
...
pub fn cmd_delete_student(
    ...
    let events = load_events()?;
    let eid = resolve_entity_id(name, &index)?;
    let student_events: Vec<_> = events
        .iter()
        .filter(|e| e.entity_id == eid && e.reverted_by.is_none())
        .collect();
    if !confirm {
        println!("⚠️ 需要使用 --confirm 确认");
        println!("   学生: {} | 事件: {} 条", name, student_events.len());
        return Ok(());
    }
    if dry_run {
        println!("[DRY-RUN] 删除: {} 事件:{}", name, student_events.len());
        return Ok(());
    }
    let now = chrono::Local::now()
        .format("%Y-%m-%dT%H:%M:%S%:z")
    ...
    save_name_index(&index)?;
    let _ = append_operation_log(
        &serde_json::json!({"action":"delete_student","entity_id":eid,"name":name,"reason":reason,"timestamp":now}),
    );
    println!(
        "✓ 学生已删除: {} (保留{}条历史事件)",
        name,
    ...
    match format {
        "csv" => {
            ...
            if out_file == "-" {
                ...
                std::fs::write(out_file, &csv)?;
                println!("✓ CSV已导出: {}", out_file);
            }
        }
        "jsonl" => {
        ...
        "html" => {
            ...
            if out_file == "-" {
                ...
                println!("✓ HTML已导出: {}", out_file);
            }
        }
        _ => {
            return Err(AppError::Validation(format!(
                "未知导出格式: {}。支持: csv, jsonl, html",
                format
            )))
        }
    }
...
pub fn cmd_summary(
    ...
    let ctx = DataContext::load()?;
    let valid_events: Vec<&Event> = ctx
        .events
        .iter()
        .filter(|e| {
            if !e.is_valid || e.reverted_by.is_some() {
                return false;
            }
            let date = if e.timestamp.len() >= 10 {
                &e.timestamp[..10]
            } else {
                return true;
            };
    ...
    let bonus_total: f64 = valid_events
        .iter()
        .filter(|e| e.score_delta > 0.0)
        .map(|e| e.score_delta)
        .sum();
    let deduct_total: f64 = valid_events
        .iter()
        .filter(|e| e.score_delta < 0.0)
        .map(|e| e.score_delta)
        .sum();
    ...
    risk_dist.insert("极高", 0usize);
    risk_dist.insert("高", 0);
    risk_dist.insert("中", 0);
    risk_dist.insert("低", 0);
    for score in ctx.scores.values() {
    ...
    let mut deltas: Vec<(&String, f64)> = ctx
        .scores
        .iter()
        .map(|(k, v)| (k, *v - BASE_SCORE))
        .collect();
    deltas.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    let top_gainers: Vec<serde_json::Value> = deltas
        .iter()
        .take(5)
        .map(|(eid, d)| {
            let name = ctx
        ...
        .collect();
    let top_losers: Vec<serde_json::Value> = deltas
        .iter()
        .rev()
        .take(5)
        .map(|(eid, d)| {
    ...
    match output {
        ...
        OutputMode::Text => {
            ...
            println!(
                "║ 扣分:       {:>4}次 总计{:+.1}          ║",
                deduct_count, deduct_total
            );
            println!("╠══════════════════════════════════════╣");
            println!("║ 风险分布:");
            for (k, v) in &risk_dist {
                println!("║   {:<10}{:>3}人", k, v);
            }
            println!("╠══════════════════════════════════════╣");
            println!("║ TOP原因码:");
            for (code, count) in top_codes.iter().take(5) {
                println!("║   {:<28}{:>3}次", code, count);
            }
            println!("╚══════════════════════════════════════╝");
        }
...
pub fn cmd_set_student_meta(
    ...
    if let Some(g) = group {
        if !entity.groups.contains(&g.to_string()) {
            entity.groups.push(g.to_string());
        }
    }
    if let Some(r) = role {
        if !entity.roles.contains(&r.to_string()) {
            entity.roles.push(r.to_string());
        }
    }
    if let Some(c) = class_id {
        entity.class_id = Some(c.to_string());
    }
...
fn generate_dashboard_html(
    ...
    let _valid_events: Vec<&Event> = ctx
        .events
        .iter()
        .filter(|e| e.is_valid && e.reverted_by.is_none())
        .collect();
    ...
    risk_dist.insert("低", 0);
    for score in ctx.scores.values() {
        *risk_dist.get_mut(risk_level(*score)).unwrap() += 1;
    }
    ...
    for (i, (eid, score)) in sorted.iter().enumerate() {
        let name = ctx
            ...
            .map(|s| s.as_str())
            .unwrap_or("?");
        let cls = match risk_level(**score) {
            "极高" => "risk-extreme",
            "高" => "risk-high",
            "中" => "risk-mid",
            _ => "risk-low",
        ...
        rows.push_str(&format!(
            "<tr><td>{}</td><td>{}</td><td>{:.1}</td><td class=\"{}\">{}</td></tr>\n",
            i + 1,
            name,
            score,
            cls,
...
pub fn cmd_doctor(output: OutputMode) -> Result<(), AppError> {
    ...
    if data_dir.exists() {
        ...
        warn += 1;
        issues.push(format!("数据目录不存在: {}", data_dir.display()));
    }
    ...
    let ent_count = match &entities_result {
        ...
        Err(e) => {
            warn += 1;
            issues.push(format!("实体加载失败: {}", e));
            0
        }
    };
    let evt_count = match &events_result {
        Ok(ev) => {
            ok += 1;
            ev.len()
        }
    ...
    if let (Ok(entities), Ok(events)) = (&entities_result, &events_result) {
        ...
        if orphan_events > 0 {
            warn += 1;
            issues.push(format!("{} 条孤立事件(entity_id无对应实体)", orphan_events));
        } else {
            ok += 1;
        }
        ...
        for evt in events {
            let ts = if evt.timestamp.len() >= 16 {
                &evt.timestamp[..16]
            } else {
                &evt.timestamp
            };
        ...
        if max_batch > 50 {
            warn += 1;
            issues.push(format!("异常批量: 单分钟最多{}条事件（阈值50）", max_batch));
        } else {
            ok += 1;
            if max_batch > 20 {
                issues.push(format!("ℹ 批量录入: 单分钟{}条事件（正常操作）", max_batch));
            }
        }
        ...
        let mut dup_ids = 0;
        for evt in events {
            if !seen_ids.insert(&evt.event_id) {
                dup_ids += 1;
            }
        }
        if dup_ids > 0 {
            warn += 1;
            issues.push(format!("{} 个重复event_id", dup_ids));
        } else {
            ok += 1;
        }
    }
