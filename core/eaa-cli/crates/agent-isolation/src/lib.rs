...
impl AgentIsolator {
    ...
    pub fn register_agent(&self, agent_id: &str) -> Result<PathBuf, IsolationError> {
        ...
        log::info!(
            "Agent {} registered with isolated directory: {:?}",
            agent_id,
            agent_dir
        );
    ...
    pub fn write_data(
        ...
        fs::write(&file_path, data).map_err(|e| IsolationError::WriteFailed(e.to_string()))?;
    ...
    /// 数据读取（自动路由到Agent隔离目录）
    pub fn read_data(&self, agent_id: &str, filename: &str) -> Result<Vec<u8>, IsolationError> {
        // 验证文件名
        if !self.validate_filename(filename) {
            return Err(IsolationError::InvalidFilename(filename.to_string()));
        }
        ...
        fs::read(&file_path).map_err(|e| IsolationError::ReadFailed(e.to_string()))
    }
...
mod security_tests {
    ...
    fn test_agent_isolation_basic() {
        ...
        // Agent A写入数据
        isolator
            .write_data("agent_a", "data.txt", b"secret data")
            .unwrap();
