...
static SENSITIVE_PATTERNS: Lazy<HashMap<&'static str, Regex>> = Lazy::new(|| {
    ...
    m.insert(
        "token",
        Regex::new(r"(?i)(token|auth_token|access_token|refresh_token)[\s:=]+([^\s]+)").unwrap(),
    );
    m.insert(
        "secret",
        Regex::new(r"(?i)(secret|client_secret|app_secret)[\s:=]+([^\s]+)").unwrap(),
    );
    m.insert(
        "api_key",
    ...
    m.insert(
        "card",
        Regex::new(r"(?i)(card|card_number|credit_card|cc_number)[\s:=]+([\d]+)").unwrap(),
    );
    m.insert(
        "ssn",
        Regex::new(r"(?i)(ssn|social[_-]?security)[\s:=]+([\d\-]+)").unwrap(),
    );
    m.insert(
        "phone",
        Regex::new(r"(?i)(phone|mobile|cell)[\s:=]+([\d\-\+\(\)\s]+)").unwrap(),
    );
    m.insert(
        "email",
        Regex::new(r"(?i)(email|e-mail)[\s:=]+([^\s@]+@[^\s@]+\.[^\s]+)").unwrap(),
    );
    m.insert(
        "address",
        Regex::new(r"(?i)(address|addr|location)[\s:=]+(.{10,})").unwrap(),
    );
    m
});
...
impl SensitiveRedactor {
    ...
    pub fn add_custom_rule(&self, pattern: &str, replacement: &str) -> Result<(), RedactionError> {
        let regex = Regex::new(pattern).map_err(|e| RedactionError::InvalidRegex(e.to_string()))?;
    ...
    pub fn redact(&self, text: &str) -> String {
        ...
        for (_, regex) in SENSITIVE_PATTERNS.iter() {
            result = regex
                .replace_all(&result, |caps: &regex::Captures| self.mask_match(caps))
                .to_string();
        }
...
mod unit_tests {
    ...
    fn test_password_redaction() {
        ...
        assert!(
            !result.contains("my_secret_password_123"),
            "Result should not contain original password"
        );
        assert_eq!(
            result.len(),
            input.len(),
            "Length should be preserved: input={}, result={}",
            input.len(),
            result.len()
        );
    }
    ...
    fn test_credit_card_redaction() {
        ...
        assert!(
            ...
            "Result should not contain original card number"
        );
        assert_eq!(result.len(), input.len(), "Length should be preserved");
    }
    ...
    fn test_sensitive_key_in_json() {
        ...
        assert!(
            ...
            "Result should not contain original secret"
        );
        // username不应该被脱敏
        assert!(
            result.contains("john"),
            "Result should still contain username: {}",
            result
        );
    }
    ...
    fn test_phone_number_redaction() {
        ...
        assert!(
            !result.contains("138"),
            "Result should not contain phone digits"
        );
    }
...
mod integration_tests {
    ...
    fn test_log_file_redaction() {
        ...
        // 写入包含敏感信息的日志
        {
            ...
            writeln!(
                file,
                "2024-01-01 10:00:00 INFO user=admin password=secret123"
            )
            .unwrap();
        ...
        assert!(
            ...
            "Should not contain token abc123"
        );
    }
}
