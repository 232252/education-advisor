...
fn test_add_student() {
    ...
    let entities: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(data_dir.join("entities/entities.json")).unwrap())
            .unwrap();
...
fn test_add_event_dry_run() {
    ...
    let events: Vec<serde_json::Value> =
        serde_json::from_str(&fs::read_to_string(data_dir.join("events/events.json")).unwrap())
            .unwrap();
