...
pub fn show(app: &mut App, ui: &mut Ui) {
    ...
    if tasks.is_empty() {
        card(ui, &app.theme, |ui| {
            empty_state(ui, &app.theme, "⏰", "暂无定时任务");
        });
    } else {
...
fn task_row(app: &mut App, ui: &mut Ui, t: &ScheduledTask) {
    card(ui, &app.theme, |ui| {
        ui.horizontal_top(|ui| {
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    let dot = if t.enabled { app.theme.success } else { app.theme.text_faint };
                    let (r, _) = ui.allocate_exact_size(Vec2::new(8.0, 8.0), egui::Sense::hover());
                    ui.painter().circle_filled(r.center(), 4.0, dot);
                    ui.label(egui::RichText::new(&t.name).font(FontId::proportional(14.0)).strong().color(app.theme.text));
                });
                ui.label(
                    egui::RichText::new(format!("cron: {}  ·  代理: {}", t.cron_expr, agent_name(&t.agent_id)))
...
fn edit_dialog(app: &mut App, ui: &mut Ui) {
    ...
    egui::Window::new("编辑任务")
        ...
        .show(ui.ctx(), |ui| {
            ...
            egui::Grid::new("task_grid").num_columns(2).spacing(Vec2::new(8.0, 6.0)).show(ui, |ui| {
                ui.label("名称");
            ...
            match crate::scheduler::validate(&t.cron_expr) {
                Ok(()) => {
                    ui.label(egui::RichText::new("✓ 表达式有效").font(FontId::proportional(11.0)).color(app.theme.success));
                    if let Ok(next) = crate::scheduler::next_fire(&t.cron_expr, Utc::now()) {
                        ui.label(
                            egui::RichText::new(format!("下次触发: {}", next.format("%Y-%m-%d %H:%M")))
                                .font(FontId::proportional(11.0))
                                .color(app.theme.text_dim),
                        );
                    }
                ...
                Err(e) => {
                    ui.label(egui::RichText::new(format!("✗ {e}")).font(FontId::proportional(11.0)).color(app.theme.danger));
                }
            }
            ui.horizontal(|ui| {
