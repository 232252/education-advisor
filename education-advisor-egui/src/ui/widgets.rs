...
/// A flat card for table rows / list items.
pub fn list_item(
    ui: &mut Ui,
    theme: &Theme,
    height: f32,
...
pub fn ghost_button(ui: &mut Ui, theme: &Theme, text: &str) -> Response {
    ...
    ui.painter().text(
        ...
        text,
        FontId::proportional(13.0),
        if resp.hovered() { theme.text } else { theme.text_dim },
    );
    resp
}
...
pub fn danger_button(ui: &mut Ui, theme: &Theme, text: &str) -> Response {
    ...
    ui.painter().rect_filled(rect, Rounding::same(10.0), bg);
    ui.painter().rect_stroke(rect, Rounding::same(10.0), Stroke::new(1.0, theme.danger));
    ui.painter().text(
        rect.center(),
        Align2::CENTER_CENTER,
        text,
...
pub fn icon_button(ui: &mut Ui, theme: &Theme, icon: &str, size: f32) -> Response {
    ...
    ui.painter().rect_filled(rect, Rounding::same(size / 4.0), bg);
    ui.painter().text(
        rect.center(),
        Align2::CENTER_CENTER,
        icon,
        FontId::proportional(size * 0.48),
        if hover { theme.accent } else { theme.text_dim },
    );
    resp
}

fn paint_button_bg(ui: &mut Ui, theme: &Theme, rect: Rect, resp: &Response, primary: bool, _danger: bool) {
    let hover = resp.hovered();
    let active = resp.is_pointer_button_down_on();
    if primary {
        let lift = if active { -1.0 } else if hover { -2.0 } else { 0.0 };
        let r = rect.translate(Vec2::new(0.0, lift));
        let top = if active { theme.accent_strong } else { theme.accent_hover };
        let bottom = if active { theme.accent } else { theme.accent_strong };
        ui.painter().rect_filled(r, Rounding::same(10.0), bottom);
        let sheen = Rect::from_min_max(r.min, Pos2::new(r.max.x, r.min.y + r.height() * 0.5));
        ui.painter().rect_filled(sheen, Rounding::same(10.0), Theme::lerp(bottom, top, 0.5));
        if !active {
...
fn button_size(ui: &Ui, text: &str, theme: &Theme) -> Vec2 {
    let galley = ui
        .painter()
        .layout(text.to_string(), FontId::proportional(13.0), theme.text, f32::INFINITY);
...
pub fn text_input(ui: &mut Ui, theme: &Theme, text: &mut String, hint: &str, width: f32) -> Response {
    let height = 34.0;
...
pub fn text_input_multiline(
    ...
    let min_h = (rows as f32 * 20.0 + 20.0).max(60.0);
...
pub fn search_input(ui: &mut Ui, theme: &Theme, text: &mut String, hint: &str, width: f32) -> Response {
    let height = 34.0;
    let (rect, mut resp) = ui.allocate_exact_size(Vec2::new(width, height), Sense::click());
    paint_input_bg(ui, theme, rect, resp.hovered() || resp.has_focus());
    let icon_w = 28.0;
        ...
        + Copy,
{
    ...
    let right_btn = Rect::from_min_size(Pos2::new(rect.max.x - btn_w, rect.min.y), Vec2::new(btn_w, height));
    let center_rect = Rect::from_min_size(Pos2::new(rect.min.x + btn_w + 2.0, rect.min.y + 6.0), Vec2::new(inner_w, height - 12.0));
    ...
    let mut inner = ui.child_ui(center_rect, egui::Layout::left_to_right(egui::Align::Center));
    let inner_resp = inner.add(edit);
...
fn paint_input_bg(ui: &mut Ui, theme: &Theme, rect: Rect, focused: bool) {
    ...
    ui.painter().rect_stroke(rect, Rounding::same(10.0), Stroke::new(1.0, stroke));
}
...
pub fn toggle_switch(ui: &mut Ui, theme: &Theme, value: &mut bool) -> Response {
    ...
    ui.painter().rect_filled(rect, Rounding::same(height / 2.0), bg);
    let margin = 3.0;
    ...
    ui.painter().circle_filled(Pos2::new(thumb_x, center_y), thumb_r, Color32::WHITE);
    resp
}
...
pub fn tab_switcher(ui: &mut Ui, theme: &Theme, tabs: &[&str], active: usize) -> Option<usize> {
    ...
    ui.painter().rect_filled(rect, Rounding::same(8.0), theme.surface);
    let count = tabs.len();
    let tab_w = total_w / count.max(1) as f32;
    let mut changed = None;
    for (i, label) in tabs.iter().enumerate() {
        let tr = Rect::from_min_size(
            Pos2::new(rect.min.x + i as f32 * tab_w, rect.min.y),
        ...
        if i == active {
            ui.painter().rect_filled(tr.shrink(2.0), Rounding::same(6.0), theme.bg_elevated);
            ui.painter().rect_stroke(tr.shrink(2.0), Rounding::same(6.0), Stroke::new(1.0, theme.border));
        } else if resp.hovered() {
            ui.painter().rect_filled(tr.shrink(2.0), Rounding::same(6.0), theme.surface_hover);
        }
        if resp.clicked() {
            changed = Some(i);
        }
        ui.painter().text(
            ...
            *label,
            ...
            if i == active { theme.text } else { theme.text_dim },
        );
    }
    changed
}
...
pub fn labeled_dropdown(
    ...
    ui.horizontal(|ui| {
        ui.label(egui::RichText::new(label).font(FontId::proportional(13.0)).color(theme.text_dim));
...
pub fn slider_f32(
    ...
    ui.painter().rect_filled(track_rect, Rounding::same(track_h / 2.0), theme.surface);
    let fill_rect = Rect::from_min_size(
        track_rect.min,
        Vec2::new(track_rect.width() * frac, track_h),
    );
    ui.painter().rect_filled(fill_rect, Rounding::same(track_h / 2.0), theme.accent);

    // thumb
    let thumb_r = 8.0;
    let thumb_x = rect.min.x + frac * rect.width();
    ui.painter().circle_filled(Pos2::new(thumb_x, track_y), thumb_r, theme.accent);
    ui.painter().circle_filled(Pos2::new(thumb_x, track_y), thumb_r - 3.0, theme.bg_elevated);
...
pub fn progress_bar(ui: &mut Ui, theme: &Theme, frac: f32, height: f32, color: Color32) {
    ...
    ui.painter().rect_filled(rect, Rounding::same(height / 2.0), theme.surface);
    let fill_w = rect.width() * frac.clamp(0.0, 1.0);
    if fill_w > 0.0 {
        let fill = Rect::from_min_size(rect.min, Vec2::new(fill_w, rect.height()));
        ui.painter().rect_filled(fill, Rounding::same(height / 2.0), color);
    }
}
...
pub fn status_pill(ui: &mut Ui, _theme: &Theme, dot: &str, label: &str, color: Color32) -> Response {
    let text = format!("{dot} {label}");
    let galley = ui
        .painter()
        .layout(text, FontId::proportional(12.0), color, 200.0);
    ...
    ui.painter().rect_stroke(
        rect,
        8.0,
        Stroke::new(1.0, Color32::from_rgba_premultiplied(color.r(), color.g(), color.b(), 50)),
    );
    ui.painter().galley(Pos2::new(rect.min.x + pad.x, rect.min.y + pad.y), galley, color);
    resp
}
...
pub fn empty_state(ui: &mut Ui, theme: &Theme, icon: &str, text: &str) {
    ui.vertical_centered(|ui| {
        ui.add_space(40.0);
        ui.label(egui::RichText::new(icon).font(FontId::proportional(44.0)).color(theme.text_faint));
        ui.add_space(10.0);
        ui.label(
            egui::RichText::new(text)
...
pub fn stat_card(
    ...
    ui.painter().rect_filled(rect, Rounding::same(16.0), theme.surface_glass);
    ui.painter().rect_stroke(rect, Rounding::same(16.0), Stroke::new(1.0, theme.border));
    ...
    let icon_rect = Rect::from_min_size(Pos2::new(rect.min.x + 14.0, rect.min.y + 14.0), Vec2::splat(40.0));
    let icon_bg = Color32::from_rgba_premultiplied(accent.r(), accent.g(), accent.b(), 30);
    ui.painter().rect_filled(icon_rect, Rounding::same(12.0), icon_bg);
    ui.painter().text(
        icon_rect.center(),
        Align2::CENTER_CENTER,
        icon,
...
pub fn checkbox(ui: &mut Ui, theme: &Theme, value: &mut bool, label: &str) -> Response {
    ...
    ui.painter().rect_filled(rect, Rounding::same(5.0), theme.surface);
    ui.painter().rect_stroke(rect, Rounding::same(5.0), Stroke::new(1.5, if *value { theme.accent } else { theme.border }));
    if *value {
        let pad = 4.0;
        let check = rect.shrink(pad);
        ui.painter().rect_filled(check, Rounding::same(3.0), theme.accent);
    }
    if !label.is_empty() {
        ui.painter().text(
            Pos2::new(rect.max.x + 8.0, rect.center().y),
...
/// A popup menu anchored to the given response. Returns the selected index if any.
pub fn popup_menu(
    ui: &mut Ui,
    _theme: &Theme,
    id: impl std::hash::Hash,
...
pub fn nav_item(
    ...
    let icon_x = if collapsed { rect.center().x } else { rect.min.x + 22.0 };
    let center_y = rect.center().y;
    let icon_align = if collapsed { Align2::CENTER_CENTER } else { Align2::CENTER_CENTER };
    ui.painter().text(
        Pos2::new(icon_x, center_y),
        icon_align,
        icon,
        FontId::proportional(if collapsed { 19.0 } else { 17.0 }),
        if active { theme.accent } else if hover { theme.text } else { theme.text_dim },
    );

    if !collapsed {
        let alpha = expanded.clamp(0.0, 1.0);
        let color = if active { theme.text } else if hover { theme.text } else { theme.text_dim };
        let c = with_alpha(color, alpha);
        ui.painter().text(
            Pos2::new(icon_x + 26.0, center_y),
            Align2::LEFT_CENTER,
            label,
            FontId::proportional(13.0),
            c,
    ...
    if let Some(b) = badge {
        let galley = ui.painter().layout(b.to_string(), FontId::proportional(10.0), Color32::WHITE, 100.0);
        ...
        let badge_rect = Rect::from_min_size(
            Pos2::new(badge_x, rect.min.y + 6.0),
            badge_size,
        );
