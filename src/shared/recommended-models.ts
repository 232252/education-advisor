// =============================================================
// 推荐的本地模型列表 — 中文友好 + CPU 友好(单一来源)
// 此前 main(ollama/recommended-models) 与 renderer(Models/lib/local-models)
// 手工双写同一份数据,已出现 URL/描述/字段名三处漂移;现收敛于此,
// 主进程一键下载与渲染层推荐卡共同消费。按硬件需求分级:
// CPU入门 → CPU进阶 → GPU/大内存
// =============================================================

/** 手动下载 GGUF 的备选链接(免登录) */
interface RecommendedModelManualLink {
  label: string
  url: string
}
...
export interface RecommendedModel {
  ...
  description: string
  manualUrls: RecommendedModelManualLink[]
}
