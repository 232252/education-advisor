...
describe('NotificationCenter — 面板交互', () => {
  it('点击铃铛打开面板,显示空状态', async () => {
    renderCenter()
    fireEvent.click(screen.getByRole('button', { name: '通知中心' }))
    // 面板为懒挂载 chunk,findBy 等待其就绪
    expect(await screen.findByText('暂无通知')).toBeDefined()
  })

  it('面板显示通知标题与未读数', async () => {
    ...
    renderCenter()
    fireEvent.click(screen.getByRole('button', { name: '通知中心' }))
    expect(await screen.findByText('Agent 运行失败')).toBeDefined()
  ...
  it('点击带 target 的通知跳转并关闭面板', async () => {
