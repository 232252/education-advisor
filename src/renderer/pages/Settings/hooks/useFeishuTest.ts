...
export function useFeishuTest({
  ...
  const handleTestConnection = useCallback(async () => {
    if (!appId) {
      ...
      setFeishuTestInfo(t('page.settings.feishu.appIdRequired', '请先填写 App ID'))
      return
    }
    setFeishuTestStatus('testing')
    setFeishuTestInfo(t('page.settings.feishu.testingNow', '正在测试...'))
    ...
    if (result.success) {
      ...
      setFeishuTestInfo(
        `${t('page.settings.feishu.testSuccessPrefix', '连接成功 · 过期 ')}${result.expireSec}s`,
      )
    } else {
      setFeishuTestStatus('error')
      // 防御: 错误信息可能是对象, 统一转字符串避免 UI 显示 "[object Object]"
      setFeishuTestInfo(
        `${t('page.settings.feishu.connectFailed', '连接失败')} · ${typeof result.error === 'string' ? result.error : JSON.stringify(result.error)}`,
      )
    }
  }, [appId, setFeishuTestStatus, setFeishuTestInfo, t])
