  ...
  return (
    <details className="border-t border-gray-200 dark:border-white/[0.06]/60">
      <summary className="px-5 py-2.5 text-xs text-gray-400 dark:text-gray-500 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/30 select-none">
        {t('page.settings.feishu.advanced', '高级(Bitable 多维表格同步)')}
      </summary>
      <div className="divide-y divide-gray-200 dark:divide-gray-700/60">
        <SettingRow
          ...
          description={t(
            'page.settings.feishu.bitableAppTokenDesc',
            '飞书多维表格的 app_token(在 bitable URL 中可找到)',
          )}
        >
          <input
            type="text"
        ...
        <SettingRow
          label={t('page.settings.feishu.bitableList', 'Bitable 列表')}
          path="feishu.bitableList"
          description={t(
            'page.settings.feishu.bitableListDesc',
            '点击拉取 Bitable 下所有表,验证凭证有效性',
          )}
        >
          <div className="flex items-center gap-2">
            <button
              type="button"
              aria-label={t('page.settings.feishu.listBitable', '拉取 Bitable 表列表')}
            ...
            >
              {bitableListStatus === 'listing'
                ...
                : t('page.settings.feishu.listBtn', '拉取列表')}
            </button>
          </div>
          {bitableListInfo && (
            <div
        ...
        <SettingRow
          label={t('page.settings.feishu.bitableTableId', 'Bitable 表 ID')}
          path="feishu.bitableTableId"
          description={t(
            'page.settings.feishu.bitableTableIdDesc',
            "同步目标表的 table_id(在多维表格 URL 中可找到,如 tblXXXXXXXX)。留空则用默认表 'log'",
          )}
        >
          <input
            type="text"
        ...
        <SettingRow
          ...
          description={t(
            'page.settings.feishu.syncIntervalDesc',
            '自动同步的时间间隔，支持 cron 表达式（如 0 */6 * * *）或分钟数',
          )}
        >
          <input
            type="text"
