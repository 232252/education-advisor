...
export const agentApi: AgentAPI = {
  // [r] 列出所有 agent
  list: () => ipcInvoke(IPC.IPC_AGENT_LIST),

  // [r] 获取单个 agent 配置
  get: (id: string) => ipcInvoke(IPC.IPC_AGENT_GET, id),

  // [w] 启用/停用 agent(持久化)
  toggle: (id: string, enabled: boolean) => ipcInvoke(IPC.IPC_AGENT_TOGGLE, id, enabled),

  // [w] 更新 Agent 配置
  update: (id: string, patch: unknown) => ipcInvoke(IPC.IPC_AGENT_UPDATE, id, patch),
  ...
  runManual: (id: string, prompt: string, history?: Array<{ role: string; content: string }>) =>
    ipcInvoke(IPC.IPC_AGENT_RUN_MANUAL, id, prompt, history),

  // [c] 中断 agent 执行
  abort: (id: string) => ipcInvoke(IPC.IPC_AGENT_ABORT, id),
