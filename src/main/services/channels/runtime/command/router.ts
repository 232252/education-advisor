// =============================================================
// channels/runtime/command/router — 斜杠命令路由器
// (M2 从 feishu-bot/command-router.ts 上提,行为不变;原文件改为 re-export 壳)
// "/命令 参数" 路由与渠道无关:命令批在 chat-queue 直通快路径分发。
//
// 设计:
//   - parseCommand():  纯函数,只做文本解析(可单测,无副作用)
//   - CommandRouter:   可扩展注册表,register/dispatch
//   - CommandContext:  能力注入接口(EAA / Agent 列表 / Agent 运行)
//                      由各渠道装配方提供实现,避免本模块直接依赖
 ...
 *   parseCommand('/score 张三')      → { command: 'score', args: ['张三'], rawArgs: '' }
...
/**
 * 命令运行所需的能力上下文。
 * 由各渠道装配方在构造时注入实现,避免本模块直接 import 主进程单例。
 */
...
const TEXT_PREVIEW_LIMIT = 1800 // 单条文本消息建议上限 ~4000 字符,留余量
