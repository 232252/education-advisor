...
export const feishuManifest: ChannelManifest = {
  ...
  configSchema: [
    ...
    },
    {
      name: 'allowFrom',
      label: '允许的用户/会话 ID',
      description: '逗号分隔 open_id/chat_id;与 aclDm/aclGroup=allowlist 联用。留空且策略为 open 则不限制',
      type: 'string',
      default: '',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      default: 'open',
      description: 'open | allowlist | deny(allowGroups=false 时强制 deny)',
    },
    {
      name: 'requireMention',
      label: '群聊需 @机器人',
      description: '对齐 QwenPaw require_mention/group_at_only;关闭后群内任意消息可进(仍受 ACL 约束)',
      type: 'boolean',
      default: true,
    },
    {
      name: 'debounceMs',
      label: '入站合并窗口(ms)',
      description: '同一会话短时多条文本合并后再进 Agent(QwenPaw debounce)。0 关闭;队列内另有批合并窗口',
      type: 'number',
      default: 0,
    },
  ],
