...
interface StudentHandlersContext {
...
const labelFor = (channel: string) => {
  ...
  return (...args: any[]) => `${channel} failed for "${args[0]}"`
}
...
export function registerStudentHandlers({
  ...
  handleIpc(
    ...
    { onError: eaaReject, timer: 'eaa:score', label: labelFor('eaa:score') },
  )
  ...
  handleIpc(
    ...
    { onError: eaaReject, timer: 'eaa:history', label: labelFor('eaa:history') },
  )

  // ----- add-student: 添加学生 -----
  // 注意: 不产生 JSON 输出
  handleIpc(
    IPC.IPC_EAA_ADD_STUDENT,
    async (_e, name: string) => {
      const safeName = sanitizeName(name, 'name')
    ...
    { onError: eaaReject, label: labelFor('eaa:add-student') },
  )
  ...
  handleIpc(
    ...
    { onError: eaaReject, label: labelFor('eaa:delete-student') },
  )
  ...
  handleIpc(
    IPC.IPC_EAA_SET_STUDENT_META,
    async (_e, params: SetStudentMetaParams) => {
