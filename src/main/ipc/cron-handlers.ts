...
export function registerCronHandlers(win: BrowserWindow) {
  ...
  handleIpc(IPC.IPC_CRON_ADD, async (_e, task: unknown) => {
  ...
  handleIpc(
    ...
    {
      label: (id: string) => `cron:update failed for "${id}"`,
    },
  )

  // H-3 修复: 加 try-catch
  handleIpc(IPC.IPC_CRON_REMOVE, (_e, id: string) => cronService.removeTask(id), {
    label: (id: string) => `cron:remove failed for "${id}"`,
  })

  // H-3 修复: 加 try-catch
  handleIpc(
    IPC.IPC_CRON_TOGGLE,
    (_e, id: string, enabled: boolean) => cronService.toggleTask(id, enabled),
    {
      label: (id: string) => `cron:toggle failed for "${id}"`,
    },
  )
  ...
  handleIpc(
    ...
    },
    {
      ...
      label: (id: string) => `cron:run-now failed for "${id}"`,
    },
  )

  // H-3 修复: 加 try-catch
  // F3 修复: catch 返回 [](renderer 契约是 CronLogEntry[]),此前的 {success:false,...} 对象与类型不符
  handleIpc(IPC.IPC_CRON_GET_LOGS, (_e, taskId?: string) => cronService.getLogs(taskId), {
    onError: () => [],
    label: (taskId?: string) => `cron:get-logs failed for "${taskId ?? 'all'}"`,
  })
