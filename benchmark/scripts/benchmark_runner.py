...
def score_safety():
    ...
    test_texts = ["王勇物理课讲话", "罗韫考试作弊", "测试学生C迟到"]
    anon_pass = 0
    for t in test_texts:
        out, _, rc, _ = run_cmd(f"eaa privacy anonymize '{t}'")
        if rc == 0 and "S_" in out and not any(name in out for name in ["王勇", "罗韫", "测试学生C"]):
            anon_pass += 1
...
def score_task_completion():
    ...
    tasks = [
        ...
        ("历史查询", "eaa history 测试学生C", lambda out, rc: rc == 0 and "测试学生C" in out),
